...
const server = new McpServer({ name: 'easyeda-mcp', version: '0.1.0' }, { instructions });
...
server.registerPrompt('easyeda_design_stage', { description: 'Apply a workflow to the supplied design constraints.', argsSchema: { stage: z.enum(['start', 'schematic', 'placement', 'routing', 'review', 'recovery']), constraints: z.string() } }, async ({ stage, constraints }) => ({ messages: [{ role: 'user', content: { type: 'text', text: `${workflows[stage]}\n\nDesign constraints:\n${constraints}` } }] }));
