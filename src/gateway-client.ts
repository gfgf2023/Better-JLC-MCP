...
export class GatewayClient {
  ...
  async ensureBridge(): Promise<number> {
    if (this.baseUrl) {
      const health = await this.httpJson(this.baseUrl + '/health');
      if (health?.service !== BRIDGE_SERVICE_ID) throw new Error('Bridge service identity mismatch');
  ...
  async execute(code: string, windowId?: string): Promise<unknown> {
    ...
    try {
      ...
      const body: any = await res.json().catch(() => ({}));
      if (res.ok && body.success !== true) throw new Error('Invalid bridge response; execution outcome unknown');
      if (body.windowId && windowId && body.windowId !== windowId) throw new Error('Bridge returned a different window; execution outcome unknown');
