---
name: better-jlc-mcp
description: Design, edit, route, or verify EasyEDA (嘉立创EDA) schematics and two-layer PCBs using Better-JLC-MCP. Use for model-directed circuit design with the official Run API Gateway, including component selection, pin connections, placement, explicit routing, and recovery. Also use when the user requests this MCP's local setup or troubleshooting.
---
...
2. Call `eda_workflow` with the current stage: `start`, `schematic`, `placement`, `routing`, `review`, or `recovery`. Read `eda_memory` and capabilities when needed. Method existence alone does not prove behavior.
3. Discover additional tools with `eda_find_tools`, read their complete `eda_tool_schema`, then use `eda_invoke` with `{name, arguments}`. Tool names below are server-local names; use the actual client namespace. Discovery is a literal substring search, so use names/categories such as `schematic`, `routing`, or `verification`.
4. Read only the relevant reference: [design workflow](references/design.md) for schematic/layout/routing/review; [recovery](references/recovery.md) for uncertain outcomes; [local setup](references/local-setup.md) when the MCP or bridge is unavailable.
